    ...
    carouselContent = carouselContent + `
      <div class="carousel-item">
        <div style="display: block; width: 100%; height: 100%; text-decoration: none; position: relative;">
          ...
          <div style="position: absolute; bottom: 20px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.7); color: white; padding: 15px 25px; border-radius: 5px; text-align: center; font-size: 16px; font-weight: bold;">${photo.lieu} | ${photo.date}</div>
          <div style="position: absolute; top: 20px; right: 2%; transform: translateX(-50%); background: rgba(0,0,0,0.7); color: white; padding: 15px 25px; border-radius: 5px; text-align: center; font-size: 16px; font-weight: bold;">Photo ${photoCounter} / ${animal.photos.length}</div>
        </div>
      </div>`;
  }
...
function displayAnimalDetails(animal) {
  ...
  animalDetailsContainer.innerHTML = `
    ...
    <div class="back-btn" style="width: 100%; justify-content: center; padding: 1em;">
